const apiKey = 'fe4d8bc132a06db690ae7d80bd8988ae'; // Replace with your OpenWeatherMap API key
...
function getWeather(city) {
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${fe4d8bc132a06db690ae7d80bd8988ae}&units=imperial`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            return fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${city}&appid=${fe4d8bc132a06db690ae7d80bd8988ae}&units=imperial`);
        })
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok');
