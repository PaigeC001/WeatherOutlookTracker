...
function displayForecast(data) {
    ...
    // Loop through the forecast data to get the next 5 days excluding today
    ...
    // Check if we have 5 days of forecasts, if not add additional days
    if (dailyForecasts.length < 5) {
        for (let i = 0; i < data.list.length; i++) {
            const forecast = data.list[i];
            const forecastDate = new Date(forecast.dt * 1000);
            ...
            if (forecastDate > new Date(dailyForecasts[dailyForecasts.length - 1].dt * 1000)) {
                dailyForecasts.push(forecast);
            }
